...
def generate_label(data_path):
    ...
    next_close = list()
    for i in range(len(df['close']) - 1):#702行   702-1=701
...
# 生成训练和测试数据
def generate_model_data(data_path,alpha,days):#days=15 temp.csv,15天为一个时间步
    df = pd.read_csv(data_path)
    ...
    #print(X_data)
    X_data = np.reshape(np.array(X_data),(-1,5*15))# 5表示特征数量*天数  #687行 15*5列 二维数组
    print(X_data)
...
def lstm_model(X_train, Y_train, X_test, Y_test):
    ...
    model.add(Dense(1,input_dim=(15,20),activation='hard_sigmoid'))##########原本没有输入维度
